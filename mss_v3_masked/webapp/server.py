...
APP_TITLE = "Tiny Mask MSS Console"
...
def _checkpoint_roots() -> list[Path]:
    ...
    roots.extend(
        [
            ROOT / "runs" / "tiny-mask-v3" / "checkpoints",
            ROOT / "export_checkpoint-v3",
            ROOT.parent / "export_checkpoint-v3",
            ROOT.parent / "runs" / "tiny-mask-v3" / "checkpoints",
            Path.cwd() / "export_checkpoint-v3",
            Path.cwd() / "runs" / "tiny-mask-v3" / "checkpoints",
...
def _find_checkpoints() -> list[dict[str, Any]]:
    ...
    checkpoints.sort(
        key=lambda item: (
            0 if "v3" in item["path"].lower() else 1,
            item["name"] != "best.pt",
            item["name"] != "last.pt",
            -item["modified"],
        )
...
def _parse_training_log() -> dict[str, Any]:
    candidates = [
        ...
        ROOT.parent / "export_checkpoint-v3" / "train_v3.log",
        ROOT / "export_checkpoint" / "train_watch.log",
        ROOT / "export_checkpoint-v3" / "train_v3.log",
        ROOT.parent / "logs" / "train_watch.log",
        ROOT.parent / "logs" / "train_v3.log",
        ROOT / "logs" / "train_watch.log",
        ROOT / "logs" / "train_v3.log",
    ]
