...
class _WatchProgress:
    def __init__(self, iterable, desc=None, disable=False, **kwargs):
        ...
        try:
            ...
            self.total = "?"
        self.every = int(os.environ.get("PROGRESS_EVERY", "5"))

    def __iter__(self):
        if not self.disable:
            print(f"{self.desc} started total={self.total}", flush=True)
        for item in self.iterable:
            self.count += 1
            if not self.disable and self.every > 0 and self.count % self.every == 0:
                elapsed = time.time() - self.started
                print(f"{self.desc} batch={self.count}/{self.total} elapsed={elapsed:.1f}s", flush=True)
            yield item
    ...
    def set_postfix(self, **kwargs):
        ...
        elapsed = time.time() - self.started
        print(f"{self.desc} batch={self.count}/{self.total} elapsed={elapsed:.1f}s " + " ".join(parts), flush=True)

def tqdm(iterable, desc=None, disable=False, **kwargs):
    return _WatchProgress(iterable, desc=desc, disable=disable, **kwargs)
...
def main() -> None:
    ...
    parser.add_argument("--out", default="runs/tiny-mask-v3")
