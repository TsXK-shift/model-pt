...
OUT_DIR="${OUT_DIR:-/kaggle/working/runs/tiny-mask-v3}"
LOG_DIR="${LOG_DIR:-/kaggle/working/logs}"
LOG_FILE="${LOG_FILE:-${LOG_DIR}/train_v3.log}"
NPROC="${NPROC:-2}"
RESUME="${RESUME:-}"

if [ ! -f "${PROJECT_DIR}/scripts/train_watch.py" ]; then
  echo "ERROR: train_watch.py not found in ${PROJECT_DIR}. Run kaggle_setup_v3.sh first."
  exit 1
fi

if [ ! -d "${MUSDB_DIR}/train" ] || [ ! -d "${MUSDB_DIR}/test" ]; then
  echo "ERROR: MUSDB18-HQ not found in ${MUSDB_DIR}. Run kaggle_setup_v3.sh first."
  exit 1
fi
